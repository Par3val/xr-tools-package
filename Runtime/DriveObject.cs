...
public class DriveObject : MonoBehaviour
{
  ...
  {
    float lerpValue = Mathf.Lerp(-dirDriveFacade.DriveLimit, dirDriveFacade.DriveLimit, targetValue);

    switch (dirDriveFacade.DriveAxis)
    {
      case DriveAxis.Axis.XAxis:
        transform.GetChild(0).localPosition = new Vector3(lerpValue, 0, 0);
        break;
      case DriveAxis.Axis.YAxis:
        transform.GetChild(0).localPosition = new Vector3(0, lerpValue, 0);
        break;
      case DriveAxis.Axis.ZAxis:
        transform.GetChild(0).localPosition = new Vector3(0, 0, lerpValue);
        break;
    }
  }
  ...
  public void SetPreviewDriveRotation(float targetValue)
  {
    ...
    {
      case DriveAxis.Axis.XAxis:
        transform.GetChild(0).localEulerAngles = new Vector3(lerpValue, 0, 0);
        break;
      case DriveAxis.Axis.YAxis:
        transform.GetChild(0).localEulerAngles = new Vector3(0, lerpValue, 0);
        break;
      case DriveAxis.Axis.ZAxis:
        transform.GetChild(0).localEulerAngles = new Vector3(0, 0, lerpValue);
        break;
    }
  }


  public void ResetDrivePosition() { transform.GetChild(0).localPosition = Vector3.zero; }
...
public class DriveObjectInspector : Editor
{
	...
	public void OnSceneGUI()
  {
    ...
    if (drive.previewBool)
      drive.SetPreviewDrivePosition(previewValue);
    else
  ...
  void ShowDriveLimit()
  {
    ...
    {
      DirectionalDriveFacade facade = drive.dirDriveFacade;
      facade.DriveLimit = EditorGUILayout.FloatField("Drive Limit", facade.DriveLimit);

      previous = facade.DriveSpeed;

      if (previous != facade.DriveSpeed)
        drive.ResetDrivePosition();
    ...
    else if (drive.driveType == DriveObject.DriveType.Rotational)
    {
      ...
      Vector2 newLimits = EditorGUILayout.Vector2Field("Drive Limits", new Vector2(facade.DriveLimit.minimum, facade.DriveLimit.maximum));
      facade.DriveLimit = new FloatRange(newLimits.x, newLimits.y);
